...
type Food = { id: number; food_name: string; quantity: string; location: string; status: string; delivery_qr_token?: string | null };
...
export default function NGOs() {
  ...
  return <div className="ngos-page">
    ...
    <main className="ngos-content">
      ...
      <section className="ngo-console"><div><span className="ngos-eyebrow">DELIVERY CONTROL</span><h2>Accept & assign incoming food</h2><p>Acceptance activates the delivery QR and starts Telegram updates for the receiver.</p></div><div className="ngo-console-fields"><input placeholder="Receiver name" value={receiverName} onChange={(e) => setReceiverName(e.target.value)} /><input placeholder="Telegram chat ID" value={telegramChatId} onChange={(e) => setTelegramChatId(e.target.value)} /><input placeholder="Delivery partner" value={deliveryPerson} onChange={(e) => setDeliveryPerson(e.target.value)} /></div>{message && <div className="ngo-console-message">✓ {message}</div>}{activated && <div className="activated-pass"><div><b>Delivery QR activated · FD{String(activated.id).padStart(4, "0")}</b><span>Show this QR to the assigned delivery partner. Scanning opens live tracking.</span></div><img src={qrUrl(activated.id, activated.delivery_qr_token)} alt="Activated delivery QR" /></div>}<div className="incoming-list">{incoming.length ? incoming.map((food) => <div className="incoming-item" key={food.id}><div><b>FD{String(food.id).padStart(4, "0")} · {food.food_name}</b><span>{food.quantity} · {food.location}</span></div><button type="button" onClick={() => acceptDonation(food.id)}>Accept & Activate QR</button></div>) : <p className="no-incoming">No available donations waiting for acceptance.</p>}</div></section>
